...
\PTComment{I re-worded this para. See what you think.} Conceptually, the \psharp test harness randomly chooses (uniformly) an input history from a set that contains all possible input histories under a certain bound \PTComment{What bound?}. Assuming that any given bug in MigratingTable can manifest under at least one of the input histories in the set (the small scope hypothesis), then an execution of the test harness must have a non-negative probability of detecting the bug.
