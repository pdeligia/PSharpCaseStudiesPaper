...
\caption{Environment model of MigratingTable (each box with a dotted line represents one \psharp machine). \PTComment{Change Reference Table to SpecTable}}
...
\PTComment{Suggested rephrasing: Instead, the developers wrote a more straightforward reference implementation of the \texttt{IChainTable} specification, called \texttt{SpecTable}, which is backed by a single in-memory table. 
 After executing an arbitrary input history on both implementations, the outputs can be compared to ensure that MigratingTable produces the same output as \texttt{SpecTable}.}
...
\PTComment{This para is slightly unclear.} The developers instrumented MigratingTable to report the intended \emph{linearization point} of each input operation on its corresponding virtual table. This allows the \psharp test harness to keep \texttt{SpecTable} in sync with the virtual table: the \texttt{Service} machine sends each input operation to \texttt{SpecTable} when the linearization point on the virtual table is reached. The output of the operation on the virtual table is then checked against its output on \texttt{SpecTable}.

\PTComment{I re-worded this para. See what you think.} The \psharp test harness randomly chooses (uniformly) an input history from a set that contains all possible input histories under a certain bound \PTComment{What bound?}. Assuming that any given bug in MigratingTable can manifest under at least one of the input histories in the set, then each execution of the test harness must have a non-negative probability of detecting the bug. \PTComment{Could re-add something about the small scope hypothesis, but it might just be noise.}

\psharp captures and controls all the interleaving in the system, and systematically explores them. To ensure that \texttt{SpecTable} is never observed to be out of sync with the backend tables, which could lead to false positives, after the \texttt{Tables} machine processes a backend operation, it enters a state that defers further backend operations until MigratingTable has reported whether the backend operation was a linearization point and (if so) \PTComment{I don't quite get this. Could you case-split here on whether the operation was or was not a linearization point?} the operation on \texttt{SpecTable} has been completed.
