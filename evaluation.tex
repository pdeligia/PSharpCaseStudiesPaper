...
% Changing ``a bug'' to ``the bug'' makes this a lot easier to read.  The risk
% of this change is that we're claiming that all failures of the P# test in each
% row represent the listed bug, rather than just ``a bug''.  I went to enough
% trouble to verify this for MigratingTable that I'm comfortable making this
% claim, although I don't have complete proof.  In any case, I don't think this
% is a distinction we can reasonably hope to get across to the reader under our
% space constraint. ~ Matt 2016-01-22
\label{tab:testing}
%\vspace{-2mm}
\end{table*}
...
% The words ``find'' and ``discover'' are misleading for the diamond bugs.
% ~ Matt 2016-01-21
