...
We argue that our approach is \emph{flexible} since it allows the user to model \emph{as much} or \emph{as little} of the environment as required to achieve the desired level of testing. We also argue that our approach is \emph{generic} since a programmer can build on top of it to test other distributed systems besides vNext (see Section~\ref{sec:cases}). Furthermore, the language features that are required to be used to connect the real code with the modeled code (e.g., \emph{virtual method dispatch}), are already being heavily used in production for testing purposes , which significantly lowers the bar for practitioners to embrace \psharp for testing.
