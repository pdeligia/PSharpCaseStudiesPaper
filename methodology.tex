...
class ExtentNodeMachine : Machine {
  ...
  void ProcessFailure() {
    ...
    PSharp.Monitor<RepairMonitor>(new ENFailedEvent(), this);
...
Figure~\ref{fig:testing_driver} illustrates how \texttt{TestingDriver} leverages \psharp to inject nondeterministic failures. It uses \texttt{Nondet()} to nondeterministically choose an \texttt{ExtentNode} machine and sends a \texttt{FailureEvent} to the chosen machine to emulate EN failure. As shown in the earlier Figure~\ref{fig:mocked_en}, the chosen \texttt{ExtentNode} machine processes the \texttt{FailureEvent},
%and instructs \texttt{ExtentManager} to expire itself by sending a \texttt{ExtentNodeDeletion} message.\texttt{ExtentNodeDeletion} is the same message that is used by the real \texttt{ExtMgr} to expire an EN. At the same time, \texttt{ExtentNode}
notifies the liveness monitor of its failure and terminates itself by invoking \texttt{PSharp.Halt()}. \psharp not only enumerates interleavings of asynchronous event handlers, but also the values returned by calls to \texttt{Nondet()}, thus enumerating different failure scenarios.
...
class RepairMonitor : Monitor {
  ...
  cold state Repaired {
    void ProcessExtentNodeFailure() {
        ...
        ExtentNodesWithReplica.Remove(node);
  ...
  hot state Repairing {
    void ProcessExtentRepaired() {
      var node = GetExtentNode(this.Payload);
      ExtentNodesWithReplica.Add(node);
      if (ReplicaCount == Harness.REPLICA_COUNT_TARGET)
        goto Repaired;
    }
  }
