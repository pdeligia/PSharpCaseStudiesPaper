...
Extent Manager uses a network engine to send and receive messages from ENs. The testing harness mocks the original network engine in vNext and overrides the networking interface. In this way, it intercepts all outbound messages and relays to the testing driver, which is responsible for dispatching the messages to destination ENs. As shown in Figure~\ref{fig:enginecode}, the mocked network engine captures outbound messages from Extent Manager and invokes \texttt{PSharpRuntime.Send(...)} to asynchronously relay the messages to the testing driver. \CHComment{Conceivably, if it is desirable to emulate message loss, the mocked network engine could leverage the non-determinism support in \psharp and drop the messages non-deterministically (only if non-determinism is already described in the previous section). }
...
Figure~\ref{fig:wrap_target} shows code snippet from Extent Manager machine, as a thin wrapper of the real Extent Manager. A mocked network engine replaces the real one in Extent Manager. It intercepts all outbound messages from Extent Manager and relays them to the testing driver.
...
Messages from ENs do {\em not} go through the mocked network engine. They are delivered to the Extent Manager machine directly and trigger an action that invokes the messages on the internal Extent Manager with \texttt{extMgr.ProcessSingleMessage}. From Extent Manager's perspective, it is unaware of the testing harness and behaves as if running in a real distributed environment and communicating with real ENs.

It is crucial to delegate all non-determinism to \psharp. For that, all internal timers of Extent Manager are disabled. Instead, the EN expiration loop and the extent repair loop are triggered repeatedly by dedicated \psharp timers. Note that system correctness should {\em not} hinge on the frequency of any individual timer. Hence, \psharp has the complete freedom to schedule arbitrary interleaving between the timers.

\subsection{Extent Node Machine}
\label{sec:method:mock_en}

The \texttt{ExtentNode} machine is reduced to the simplest. It omits the details of real Extent Node, mocks only those logic required for the testing scenarios, and keeps the mocked logic simplest. The logic here includes: repairing an extent from its replica, generating sync reports periodically, and sending heartbeat messages periodically.

The testing harness leverages components in vNext whenever appropriate. Here, the machine re-uses \texttt{ExtentCenter}, an internal component of real Extent Node, for extent bookkeeping.

In the mocked extent repair logic, the machine is instrumented by Extent Manager to repair an extent. It sends a repair request to a source Extent Node where a replica is stored. After receiving a \texttt{ExtentRepairResponse} from the source, it updates its internal \texttt{ExtentCenter}, as illustrated in Figure~\ref{fig:mocked_en}. 

In the mocked sync logic, the machine is triggered repeatedly by a \psharp timer. It prepares a sync report with \texttt{extCtr.GetSyncReport(...)} and asynchronously sends the report to Extent Manager with \texttt{PSharpRuntime.Send(...)}.
...
class ExtentNodeMachine : Machine {
  ...
  // extent repair logic
  ...
  [OnEvent(ExtentRepairResponse, ProcessRepairResponse)]
  void ProcessRepairResponse()
  {
    // repair response from source replica
    if (IsRepairSucceeded(this.Payload))
    {
...
As illustrated in Figure~\ref{fig:monitor}, the monitor machine transitions between a cold state and a hot state. It is desirable to maintain a target number of replicas for an extent. Whenever an extent node fails, the monitor is notified and transitions into the hot {\em repairing} state, as the replica number now falls below the target. Once the extent is repaired, the monitor is again notified and transitions into the cold {\em repaired} state, when the replica number reaches the target.

In the extent repair testing scenarios, it is expected that the monitor should {\em always eventually} end up in the cold repaired state. Otherwise, if the monitor is stuck in the hot repairing state for {\em infinitely} long, it is an indication that the corresponding execution sequence results in the extent never being repaired -- a liveness bug!

\begin{figure}[t]
\begin{lstlisting}
class ExtentRepairMonitor : Machine {
  // true: with replica, false: w/o replica
  ...
  state Repairing : MonitorState {
    void ProcessExtentRepaired()
    {
      ...
      if (ReplicaCount == Harness.REPLICATION_FACTOR)
        goto Repaired;
    }
  }
}
