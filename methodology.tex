...
%\begin{enumerate}
\begin{description}
\item[Extent Manager machine] is a thin wrapper for the real Extent Manager in vNext.

\item[Extent Node machines] represent simple mocked ENs.

\item[Testing Driver machine] communicates with all other machines, relays messages between the Extent Manager machine and the EN machines, and drives testing scenarios. In one scenario, the testing driver launches one Extent Manager and three ENs, with a single extent on one of the ENs. It then waits for the extent to be replicated at the other ENs. In another scenario, the testing driver fails one of the ENs and launches a new one. It then waits for the extent to be repaired on the new EN.

\item[Monitor machine] collects states from all the ENs and verifies desired properties, such as the extent is replicated or repaired at the expected ENs.
\end{description}
%\item {\em Extent Manager machine} is a thin wrapper for the real Extent Manager in vNext.
%
%\item {\em Extent Node machine} represents simple mocked EN.
%
%\item {\em Testing Driver machine} communicates with all other machines, relays messages between the Extent Manager machine and multiple EN machines, and drives testing scenarios. In one testing scenario, the testing driver launches Extent Manager with three ENs, with a single extent on one of the ENs. It then waits for the extent to be repaired on all three ENs. In another testing scenario, the testing driver fails one of the ENs and launches a new EN. It then waits for the extent to be repaired on all the remaining ENs.
%
...
The testing target, the real Extent Manager in vNext, is wrapped inside a \psharp machine in the testing harness.
...
Inside Extent Manager, there are two data structures related to extent replication and repair: an \texttt{ExtentCenter} and an \texttt{ExtentNode Map}. The extent center maintains the mapping records from extents to their hosting ENs. It is updated upon the periodic sync reports from the ENs. Again recall that the sync report from a particular EN lists all the extents stored at the EN. Its purpose is to update Extent Manager's possible out-of-date view of the EN with the ground truth. The EN map records the latest heartbeat time from every EN.

Extent Manager internally runs a periodic {\em EN expiration loop} that is responsible for removing ENs that have been missing heartbeats for an extended period, as well as cleaning up the corresponding records in the extent center. In addition, it runs a periodic {\em extent repair loop}, which examines all the extent center records, identifies extents with missing replica, schedules extent repair tasks and sends them to the ENs.

\subsubsection{Intercepting Extent Manager Messages}
...
Extent Manager uses a network engine to send messages to the ENs. The testing harness mocks the original network engine in vNext and overrides its interface. In this way, the mocked network engine intercepts all outbound messages and relays them to the testing driver, which is responsible for dispatching the messages to the destination ENs. As shown in Figure~\ref{fig:enginecode}, the mocked network engine intercepts the outbound messages from Extent Manager and invokes \texttt{PSharpRuntime.Send(...)} to asynchronously relay the messages to the testing driver. Conceivably, the mocked network engine could leverage the non-determinism support in \psharp and choose to drop the messages in a non-deterministic fashion, in case emulating message loss is desirable. \CHComment{only if non-determinism is already covered in the previous section.}
...
Figure~\ref{fig:wrap_target} shows some code snippets from the Extent Manager machine. It is a thin wrapper of the real Extent Manager. The mocked network engine replaces the real one in Extent Manager, which intercepts all the outbound messages from Extent Manager and relays them to the testing driver.
...
\end{lstlisting}
\vspace{-2mm}
\caption{Extent Manager Machine: the Testing Target.}
\label{fig:wrap_target}
%\vspace{-2mm}
\end{figure}

Messages coming from the ENs do {\em not} go through the mocked network engine. They are delivered to the Extent Manager machine directly and trigger an action that invokes the messages on the internal Extent Manager with \texttt{extMgr.ProcessSingleMessage}. From Extent Manager's perspective, it is unaware of the testing harness and behaves as if running in a real distributed environment and communicating with real ENs.

All internal timers of Extent Manager are disabled, because it is crucial to delegate all non-determinism to \psharp. Instead, the EN expiration loop and the extent repair loop are driven by external timers provided by \psharp. Note that system correctness should {\em not} hinge on the frequency of any individual timer. Hence, \psharp has the complete freedom to schedule arbitrary interleaving between the timers.

\subsection{Extent Node Machine}
\label{sec:method:mock_en}

The Extent Node machine is reduced to the simplest. It omits the details of a real Extent Node, mocks only those logic necessary for the testing scenarios, and keeps the mocked logic simplest. The logic here includes: repairing an extent from its replica, generating sync reports periodically, and sending heartbeat messages periodically.

The testing harness leverages components in vNext whenever appropriate. Here, the Extent Node machine re-uses \texttt{ExtentCenter}, an internal component of real Extent Node, for extent bookkeeping.

In the mocked extent repair logic, the machine takes action upon receiving an extent repair request from Extent Manager. It sends a copy request to a source Extent Node where a replica is stored. After receiving a \texttt{ExtentCopyResponse} from the source, it updates the internal extent center, as illustrated in Figure~\ref{fig:mocked_en}. 

In the mocked extent node sync logic, the machine is again driven by an external timer provided by \psharp. It prepares a sync report with \texttt{extCtr.GetSyncReport(...)} and asynchronously sends the report to Extent Manager with \texttt{PSharpRuntime.Send(...)}.
...
%\subsection{Testing Driver Machine}
%\label{sec:method:driver}

\subsection{Monitor Machine}
\label{sec:method:monitor}

The monitor machine transitions between a cold state and a hot state. Whenever an extent node fails, the monitor is notified. As soon as the number of extent replica falls below a specified target, the monitor transitions into the hot {\em repairing} state, where the missing replica is being repaired. Whenever a replica is repaired, the monitor is also notified. It transitions into the cold {\em repaired} state, when the replica number reaches the target again, as illustrated in Figure~\ref{fig:monitor}.

%It verifies the property that the number of extent replica {\em eventually} reaches the target.

In the extent repair testing scenarios, the monitor verifies that it should {\em always eventually} end up in the cold repaired state. Otherwise, the monitor is stuck in the hot repairing state for {\em infinitely} long. This indicates that the corresponding execution sequence results in extent replica never being repaired -- a liveness bug!

\begin{figure}[t]
\begin{lstlisting}
class ExtentRepairMonitor : Machine {
