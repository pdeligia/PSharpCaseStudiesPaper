...
The testing harness consists of multiple \psharp machines, as shown in Figure~\ref{fig:azurestoremodel}:
%\begin{enumerate}
\begin{description}
...
\caption{Real Extent Manager with a Mocked Environment (each box presents one \psharp machine).}
...
The testing driver machine drives two testing scenarios. In one scenario, it launches one Extent Manager and three ENs, with a single extent on one of the ENs. It waits for the extent to be replicated at the other ENs. In another scenario, it fails one of the ENs and launches a new one. It waits for the extent to be repaired on the new EN. Code snippets are skipped due to space constraint.
