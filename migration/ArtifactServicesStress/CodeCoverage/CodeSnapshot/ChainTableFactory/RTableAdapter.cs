...
{
    public class RTableAdapter : IChainTable  // public for demo
    {
            ...
            TableRequestOptions requestOptions = null, OperationContext operationContext = null)
        {
            ...
            // Chunzhi: the published rtable returns 204 for insert :(
            ...
            // Chunzhi: the published rtable returns 409 for precondition failure on merge / replace / delete
        ...
        // Chunzhi: the following is wrong, does not provide atomicity
        /*
            ...
            TableRequestOptions requestOptions = null, OperationContext operationContext = null)
        {
            ...
            foreach (var op in batch)
            {
                try
                {
                    res.Add(Execute(op, requestOptions, operationContext));
                }
                catch (StorageException e)
                {
                    throw new ChainTableBatchException(nowat, e);
                }
                ++nowat;
            }
            return res;
        ...
        */
            ...
            TableRequestOptions requestOptions = null, OperationContext operationContext = null)
        {
            ...
            catch (StorageException ex)
            {
                ...
                // Chunzhi: Currently RTable does not return failed index, always set to 0
                ...
                */
                ...
                // Chunzhi: change RTableConflictException to a standard StorageException with httpcode = conflict
        ...
        private TableOperation EncapsulateOp(TableOperation oldOp, OperationContext operationContext)
        {
                ...
                || opType == TableOperationType.Delete)
            {
                ...
                if (opType == TableOperationType.Insert)
                    result = TableOperation.Insert(encapsulatedRow);
        ...
        private TElement DecapsulateEnt<TElement>(DynamicReplicatedTableEntity rawEnt) where TElement : ITableEntity, new()
        {
            ...
            // Chunzhi: RTable's query interface forgot to virtualize etag, so we virtualize it here
