...
namespace Migration
{
    ...
    public enum TableClientState
    {
        ...
        // and then go to the old table unless the partition is always
        // switched.  These clients can coexist with both USE_OLD_EXPORT
        // and PREFER_NEW.
        PREFER_OLD,
    ...
    public class MTableConfiguration
    {
        ...
        [DataMember] public TableClientState state { get; set; }
    ...
    internal class NullMonitor : IChainTableMonitor
    {
        ...
        public Task AnnotateLastBackendCallAsync(bool wasLinearizationPoint = false, IList<TableResult> successfulBatchResult = null,
    ...
    internal const string ROW_KEY_NEVER_EXISTS = "_mtable_never_exists";
        ...
        protected internal readonly IReadOnlyConfigurationService<MTableConfiguration> configService;
        ...
        public MigratingTable(IReadOnlyConfigurationService<MTableConfiguration> configService,
            IChainTable2 oldTable, IChainTable2 newTable, MTableOptionalBug? bugToEnable = null)
            :this (configService, oldTable, newTable, NullMonitor.Instance, bugToEnable)
        {
        ...
        private static async Task<T> CreateTableWhileNotFoundExceptionAsync<T>(IChainTable2 table, Func<IChainTable2, Task<T>> tableAction)
        {
            try
            {
                return await tableAction(table).ConfigureAwait(false);
            }
            catch (StorageException storageException)
            {
                if ((HttpStatusCode)storageException.RequestInformation.HttpStatusCode != HttpStatusCode.NotFound)
                {
                    throw;
                }

                if (storageException.RequestInformation.ExtendedErrorInformation == null ||
                    storageException.RequestInformation.ExtendedErrorInformation.ErrorCode != "TableNotFound")
                {
                    throw;
                }
            }
        ...
        internal async Task TryCopyEntityToNewTableAsync(MTableEntity entity, TableRequestOptions requestOptions, OperationContext operationContext)
        {
            try
            {
                await CreateTableWhileNotFoundExceptionAsync(
                    newTable,
                    t => t.ExecuteAsync(TableOperation.Insert(entity), requestOptions, operationContext));
            }
            catch (StorageException ex)
            {
                if (ex.GetHttpStatusCode() != HttpStatusCode.Conflict)
                ...
                case TableOperationType.Delete:
                    ...
                    else
                    {
                        ...
                        else if ((errorCode = CheckExistingEntity(passedEntity, existingEntity)) == null)
                        {
                            ...
                            newOp = TableOperation.Delete(new MTableEntity {
                                PartitionKey = buggablePartitionKey,
                                RowKey = buggableRowKey,
        ...
        async Task<IList<TableResult>> PassthroughBatchToOldTableAsync(TableBatchOperation batch, TableRequestOptions requestOptions, OperationContext operationContext)
        {
            ...
            try
            {
                results = await oldTable.ExecuteBatchAsync(batch, requestOptions, operationContext).ConfigureAwait(false);
                return results;
            }
            finally
            {
        ...
        async Task<IList<TableResult>> AttemptBatchOnOldTableAsync(TableBatchOperation batch, TableRequestOptions requestOptions, OperationContext operationContext)
        {
            ...
            // Label X from Migration_procedure.docx
        ...
        public override async Task<IList<TElement>> ExecuteQueryAtomicAsync<TElement>(TableQuery<TElement> query, TableRequestOptions requestOptions = null, OperationContext operationContext = null)
        {
            ...
            string partitionKey = ChainTableUtils.GetSingleTargetedPartitionKey(origFilterExpr);  // validation
            ...
            using (configService.Subscribe(FixedSubscriber<MTableConfiguration>.Instance, out config))
            {
                ...
                else
                {
                    ...
                    {
                        // filterExpr must be "PartitionKey eq FOO", which already matches the meta row; nothing to do.
                    }
                    else
                    {
                        // filterExpr must be "(PartitionKey eq FOO) and (BAR)".
                        // Rewrite to "(PartitionKey eq FOO) and ((RowKey eq ROW_KEY_PARTITION_META) or (BAR))".
                ...
                var filteredResults = (from ent in results
                    where !RowKeyIsInternal(ent.RowKey) && !ent.deleted
                          && origFilterExpr.Evaluate(ent) && !ent.deleted
                    select ent.Export<TElement>());

                if (query.TakeCount.HasValue)
                {
                    filteredResults = filteredResults.Take(1);
                }
        ...
        class QueryStream<TElement> : IQueryStream<TElement>, IConfigurationSubscriber<MTableConfiguration>
            where TElement : ITableEntity, new()
        {
            // Compare to http://blogs.msdn.com/b/pfxteam/archive/2012/02/12/10266988.as
            ...
            public class AsyncLock
            {
                ...
                Task holdTask = Task.FromResult(0);
            ...
            public async Task ApplyConfigurationAsync(MTableConfiguration newConfig)
            {
                using (await LockAsyncBuggable())
                {
                    ...
                    {
                        // XXX: mtableQuery might already have a continuation filter
                        // condition as a conjunct due to layering of IChainTable2s,
                        // which would lead to some extra overhead and push us
                        // closer to the limit on number of comparisons.  Either try
                        // to parse for this or change ExecuteQueryStreamedAsync to
                        // always take a continuation primary key (in which case the
                        // next few lines move to the Azure IChainTable2 adapter)?
                        mtableQuery.FilterString = ChainTableUtils.CombineFilters(
                            ChainTableUtils.GenerateContinuationFilterCondition(oldTableNext.GetPrimaryKey()),
                            TableOperators.And,
                            mtableQuery.FilterString // Could be empty.
                            );
                        newTableStream =
                            await
                                outer.newTable.ExecuteQueryStreamedAsync(newTableContinuationQuery, requestOptions,
                                    operationContext);
                        newTableNext = await newTableStream.ReadRowAsync();
            ...
            public void Dispose()
            {
                using (LockAsyncBuggable().Result)
                {
                    if (!disposed)
                    {
                        disposed = true;
            ...
            using (configService.Subscribe(FixedSubscriber<MTableConfiguration>.Instance, out config))
            {
                switch (config.state)
                {
                    case TableClientState.USE_OLD_HIDE_METADATA:
                        return oldTable.CreateIfNotExists();
                    case TableClientState.PREFER_OLD:
                        oldTable.CreateIfNotExists();
                        return newTable.CreateIfNotExists();
                    default:
                        return newTable.CreateIfNotExists();
                }
            }
        ...
        public override bool DeleteIfExists()
        {
            return oldTable.DeleteIfExists() && newTable.DeleteIfExists();
        }
    }
}
